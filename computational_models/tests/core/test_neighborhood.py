from computational_models.core.neighborhood import Moore, VonNeumann


def test_von_neumann_size():
    assert VonNeumann.size() == 8
...
def test_von_neumann_indexes_for():
    von_neumann_neighborhood = VonNeumann(30)
    position = (10, 20)
    expected_neighbors = [
        (9, 19),
...
def test_von_neumann_border_indexes_for():
    von_neumann_neighborhood = VonNeumann(30)
    position = (0, 0)
    expected_neighbors = [
        (29, 29),
...
def test_moore_border_indexes_for():
    moore_neighborhood = Moore(20)
    position = (0, 0)
    expected_neighbors = [
        (19, 0),
